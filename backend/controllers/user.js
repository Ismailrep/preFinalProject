...
exports.register = async (req, res) => {
  try {
    ...
    if (check2) {
      return res.status(400).json({
        message:
          "This username already exists,try with a different username",
      });
    }
    ...
    if (!validateLength(username, 4, 10)) {
      return res.status(400).json({
        message: "last name must between 4 and 10 characters.",
      });
    }
    if (!validatePassword(password)) {
      return res.status(400).json({
        message: "Passwords must be contain at least 8 characters including an uppercase letter, a symbol, and a number.",
      });
    }
